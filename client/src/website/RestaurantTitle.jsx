...
export default function RestaurantTitle() {
  ...
  return (
    ...
    <p className="Rdescription"> {showMore ? vendorAboutBusiness : `${vendorAboutBusiness.substring(0, 400)}`+'...'} <a onClick={() => setShowMore(!showMore)}>{showMore ? " Show Less" : ((vendorAboutBusiness && vendorAboutBusiness.length > 400) ? " Show More" : '')}</a></p>
      </>
  );
}
