...
const LatestListing = (props) => {
  ...
  useEffect(() => {
    const getComments = async () => {
      var config1 = {
        method: 'get',
        url: '/api/latest-additions/latest/all/',
        headers: { 
   ...
   const fetchComments = async () => {
    ...
    let url = `/api/latest-additions/latest/all?country=${country}&page=${page}&size=${size}`;
  ...
  const handleLastClick = (e) => {
    ...
    var config = {
      method: 'get',
      url: '/api/latest-additions/latest/all/'+'?filter='+filter+'&country='+country+'&page='+pagesCount+'&size='+size,
      headers: { 
        'Content-Type': 'application/x-www-form-urlencoded'
      },
      params : {
